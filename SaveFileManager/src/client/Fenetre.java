...
public class Fenetre implements ActionListener {
	...
	public static JButton ajouter, enlever, benvoyer, brecevoir, bquitter, bnouveau,binfo, bsuppr;
	...
	private Container contentPane() {
		...
		bnouveau = new JButton("Nouveau",new ImageIcon(getClass().getResource("/file.png")));
			...
			bnouveau.setToolTipText("Commencer une nouvelle sauvegarde");
			add.add(bnouveau);
		ajouter = new JButton("Ajouter",new ImageIcon(getClass().getResource("/action_add.png")));
			ajouter.addActionListener(this);
			ajouter.setToolTipText("Ajouter un ou plusieurs fichier(s) à la sélection");
			add.add(ajouter);
		enlever = new JButton("Enlever",new ImageIcon(getClass().getResource("/action_delete.png")));
			enlever.addActionListener(this);
			enlever.setToolTipText("Enlever le fichier sélectionné de la sélection");
			enlever.setEnabled(false);
			add.add(enlever);	
		bsuppr = new JButton("Supprimer",new ImageIcon(getClass().getResource("/action_delete.png")));
			bsuppr.addActionListener(this);
			bsuppr.setEnabled(false);
			bsuppr.setToolTipText("Supprimer une archive sur le disque dur du serveur");
			add.add(bsuppr);	
		benvoyer = new JButton("Envoyer",new ImageIcon(getClass().getResource("/arrow_top.png")));
			...
			benvoyer.setToolTipText("Envoyer les fichiers sur le serveur");
			add.add(benvoyer);
	...
	public void actionPerformed(ActionEvent e) {
        ...
        if(i == JFileChooser.APPROVE_OPTION) {
			...
			try {
        		int j = liste.getSelectedIndex();
        		Fenetre.frame.setCursor(waitCursor);
        		new Supprimer(selection.elementAt(j));
        		Fenetre.frame.setCursor(Cursor.getDefaultCursor());
        		new WhatIDo("Supprimé",selection.elementAt(j).toString());
        		selection.removeElementAt(j);
			} catch (ArrayIndexOutOfBoundsException e1) {
				Fenetre.frame.setCursor(waitCursor);
        		new Supprimer(selection.elementAt(0));
        		Fenetre.frame.setCursor(Cursor.getDefaultCursor());
				new WhatIDo("Supprimé",selection.elementAt(0).toString());
        		selection.removeElementAt(0);
			}
        	if (selection.size()==0) {
        		selection.clear();
